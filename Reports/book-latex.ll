...
proc main() {
  ...
  getindi(indi, "Enter the first person for the report")
  if(not(indi)) {
  ...
  while(indi) {
    ...
    set(indi, 0)
    getindi(indi, "Enter another root person or none to proceed")
  }

  if(1) {
    getint(ancestormode, "Enter 0 for descendant, 1 for ancestor report")
  } else {
    set(ancestormode, 0)
  }
  ...
  if(eq(0, ancestormode)) {
    if(1) {
      getint(maxgenprint, "Max generations for descendancy books")
    } else {
      set(maxgenprint, 999)
    }
  }
  ...
  if(0) {  /* change this to  "if (1)" to turn this query on. */
    getint(dumpplacetable_each_gen,
  ...
  getstr(author, "Enter the author(s) of this document:")
  ...
  getstr(copyplace, "Enter the place for the copyright notice:")
  ...
  getstr(intro, "File that contains introduction (if any):")
