...
proc main ()
{
      ...
      getindi(ancestor, "Identify the ancestor (Optional)")
      if(ancestor) {
        getindi(descendant,"Identify the descendant (Optional)")
      }
      else {
        getindi(descendant,"Identify the descendant (Required)")
      }
      set(allanc, 0)
      if(and(ne(descendant,0),eq(ancestor,0))) {
        getint(allanc, "All Ancestors? (1 = yes, 0 = no)")
      ...
      if(and(eq(descendant,0),ne(ancestor,0))) {
        getint(gcount, "Number of Generations")
      ...
      if(and(ne(ancestor, 0),ne(descendant,0))) {
        getint(gnum, "First Generation Number (default is 1)")
        if(le(gnum,0)) { set(gnum,1) }
        set(down, 0)
        if(gt(gnum,1)) {
          getint(down, "Generations count downward (1) or upward (0)")
        }
        set(firstgen, gnum)
        if(descendant) {
