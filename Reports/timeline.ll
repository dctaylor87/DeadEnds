...
proc main ()
{
        ...
        while (eq(valid,0)) {
            ...
            print(nl())
            getint(ltype,"Choose subset of individuals: ")
            if (and(ge(ltype,1), le(ltype,6))) {
                set (valid, 1)
            }
        }
        while (and(ne(showall,1), ne(showall,2))) {
            getint(showall,"Include people without dates (1) no, (2) yes: ")
        }
        if (eq(gra,1)) {
            pagemode(1, 200)
            set(startdate,0)
            ...
            while(le(enddate, startdate)) {
                ...
                while (or(le(startdate,0),gt(startdate,MAXYEAR))) {
                  getint(startdate,"Enter start date for graph, e.g. 1800: ")
                }
                while (or(le(enddate,0),gt(startdate,MAXYEAR))) {
                    getint(enddate, "Enter end date for graph, e.g. 1950: ")
                }
            ...
            while (and(ne(size,1), ne(size,2))) {
                getint(size,"Select graph size (1) 80 col, (2) 132 col: ")
            }
            while (and(ne(order,1), ne(order,2))) {
                getint(order,"Order by (1) family group, (2) last name: ")
            }
            if (eq(size, 1)) {
