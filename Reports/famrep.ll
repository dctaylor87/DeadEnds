...
proc main ()
{
    ...
    while (or(lt(ionly,1), gt(ionly,2))) {
      getint(ionly,"Choose (1) Individual only, (2) + Married Descendents: ")
    }
    if (eq(ionly, 2)) {
        set(sonly,0)
        while (or(lt(sonly,1), gt(sonly,2))) {
            getint(sonly,"Choose (1) Select Spouse, (2) All Spouses: ")
        }
    }
    else {
        set(sonly, 1)
...
proc FGsheet(indi, ionly, sonly)
{
    if (eq(sonly, 1)) {
        ...
        if (gt(i, 1)) {                 /* select a spouse */
            getint(num, "Choose which spouse for Family Report: ")
        }
        else {
            set(num, 1)
        }
