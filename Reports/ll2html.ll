...
/* Change these to suit your needs */
    ...
    indiset(people)
    getindi(root_person,"Enter root individual:")
    set(root_key,key(root_person))
    set(clumpsize,0)
    while (le(clumpsize,0))
    {
	getint(clumpsize,"Enter number of individuals per file:")
    }
/*    getint(separate_clumps,
 ...
 */
	...
	while(person) {
	    set(person,0)
	    getindi(person,"Enter non-private person:")
    ...
    while(person) {
	set(person,0)
	getindi(person,"Enter root for descendant list:")
...
proc create_index_file(desc_roots)
{
  ...
  getint(sortit,"Sort the indexes? (0=no, 1=yes)")
  if (sortit) {
