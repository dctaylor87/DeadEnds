...
static bool pieceMatch(CString partial, CString complete);
static void nameToParts(CString name, String *parts);
static void squeeze(CString string, String super);
static CString nextPiece(CString name);
static void cmpsqueeze (CString in, String out);
...
#define NBUFFERS (4)
String getSurname(CString name) {
    int c;
...
int getFirstInitial(CString name) {
    int c;
    while (true) {
        while (iswhite(c = *name++));
...
static void squeeze(CString in, String out) {
    int c;
...
String* personKeysFromName(CString name, RecordIndex* rindex, NameIndex* nindex, int* pcount) {
...
int compareNames(CString name1, CString name2) {
...
String getGivenNames (CString name) {
    int c;
...
static CString nextPiece (CString in) {
    int c;
    while (true) {
...
static String nameSurnameFirst(CString name) {
    static char scratch[MAXNAMELEN+1];
    String p = scratch;
    ASSERT(strlen(name) <= MAXNAMELEN);
