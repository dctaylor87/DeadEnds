...
InterpType interpFamilies(PNode* pnode, Context* context, PValue *pval) {
	...
	assignValueToSymbol(context, pnode->spouseIden,
			    spouse ? PVALUE(PVPerson, uGNode, spouse)
			    : nullPValue);
...
InterpType interpretSequenceLoop(PNode* pnode, Context* context, PValue* pval) {
    ...
    FORSEQUENCE(seq, el, ncount) {
        ...
        //PValue pvalue = (PValue) {PVInt, el->value->uInt}; // Update person's value in symbol table.
