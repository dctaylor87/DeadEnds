...
bool rptui_ask_for_program(CString mode, CString ttl, CString *pfname,
			   CString path, CString ext, bool picklist);
int rptui_chooseFromArray(CString ttl, int no, CString *pstrngs);
int rptui_elapsed(void);
void rptui_init(void);
int rptui_prompt_stdout(CString prompt);
void rptui_view_array(CString ttl, int no, CString *pstrngs);
