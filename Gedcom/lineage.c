...
GNode* personToPreviousSibling(GNode* indi)
{
	...
	while (node && eqstr("CHIL", node->tag)) {
		if (eqstr(indi->key, node->value)) {  // Found the person as a child.
			...
			return keyToPerson(prev->value, theDatabase);  // There is a previous sibling.
		}
		prev = node;
...
GNode* personToNextSibling(GNode* indi)
{
	...
	return keyToPerson(node->value, theDatabase);
}
...
//-------------------------------------------------------------------------------------------------
GNode *familyToSpouse(GNode *family, SexType sex)
...
//  person -- Sex of spouse.
{
	...
	return (sex == sexMale) ? familyToHusband(family) : familyToWife(family);
}
...
//  node -- Root of a family Gedcom record.
{
	...
	return keyToPerson(chil->value, theDatabase);
}
...
GNode* personToFamilyAsChild(GNode* person)
{
	...
	return keyToFamily(person->value, theDatabase);
}
